...
with open('boxoffice.csv', 'w', encoding='utf-8', newline='') as f:
    ...
    for page in range(1,100):
        api_url = f'https://api.themoviedb.org/3/movie/popular?api_key={api_key}&language=ko-KR&page=1'
...
with open('movie.csv', 'w', encoding='utf-8', newline='') as f:
    ...
    for movie in range(len(movieList)):
        ...
        for genre in range(len(response['genres'])):
            movieInfos['genres'] = response['genres'][genre]['id']

        api_url = f'https://api.themoviedb.org/3/movie/{movieList[movie]}/credits?{api_key}'
        response = requests.get(api_url).json()
        ...
        if actorInfo:
            ...
            for actorIndex in range(val):
                if actorInfo[actorIndex]['id'] not in actors:
                    ...
                    movieInfos[f'actor_{actorIndex+1}'] = actorInfo[actorIndex]['id']
                               
        if crewInfo:
            for crew in crewInfo:
                if crew['job'] == 'Director':
                    movieInfos['director'] = crew['name']

        api_url = f'https://api.themoviedb.org/3/movie/{movieList[movie]}/images?{api_key}'         
        response = requests.get(api_url).json()
        ...
        for imgIndex in range(val):
            movieInfos[f'file_path_{imgIndex+1}'] = imgInfo[imgIndex]['file_path']
        csv_writer.writerow({
                                    'movie_id': infos['id'],
                                    'movie_title': infos['title']
                                    })
